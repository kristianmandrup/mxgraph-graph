...
export class InPlaceEditorConfig {
  configure() {
    this.setStartEditing();
    this.setInstallListeners();
    this.setToggleViewMode();
    this.setResize();
    this.setGetInitialValue();
    this.setGetCurrentValue();
    this.setStopEditing();
    this.setFocusContainer();
    this.setApplyValue();
    this.setGetBackgroundColor();
    ...
    mxCellEditor.prototype.toggleViewMode = () => {
      ...
      if (state != null) {
        ...
        if (!this.codeViewMode) {
          ...
          var content = mxUtils.htmlEntities(textarea.innerHTML);
          ...
          var content = mxUtils.extractTextWithWhitespace(textarea.childNodes);
  ...
  setResize() {
    const { textarea, codeViewMode } = this;
    var mxCellEditorResize = mxCellEditor.prototype.resize;
    mxCellEditor.prototype.resize = (state, _trigger) => {
      if (textarea != null) {
        ...
        if (codeViewMode && state != null) {
          ...
          if (textarea.clientWidth < textarea.offsetWidth) {
            textarea.style.width =
              ...
              "px";
            this.bounds.width = parseInt(textarea.style.width) * scale;
          }

          textarea.style.left = Math.round(this.bounds.x) + "px";
          textarea.style.top = Math.round(this.bounds.y) + "px";
  ...
  setStopEditing() {
    ...
    mxCellEditor.prototype.stopEditing = (_cancel) => {
